...
class BasePlane(Base):
    """飞机基类"""
    global game_end
    game_end = pygame.image.load("./feiji/game_end.png")
            ...
            for bullet in plane.bullet_list:
                if bullet.x > self.x + 0.05 * width and bullet.x < self.x + 0.95 * width and bullet.y + 0.1 * height > self.y and bullet.y < self.y + 0.8 * height:
                    ...
                    if self.plane_type == 3:
                        ...
                        while self.HP==0:
                            # 显示暂停图片
                            window_screen.blit(game_end, (150, 150))  # 背景
                            # 更新图片
...
class BaseBullet(Base):
    ...
    # 子弹显示
    def display(self):
        self.screen.blit(self.image, (self.x-15, self.y))
...
class EnemyBullet(BaseBullet):
    ...
    def __init__(self, screen_temp, x, y, plane):
        BaseBullet.__init__(self, screen_temp, x+18 + plane_size[plane.plane_type]["width"] / 2,
...
def enemy_display_move_fire(enemy):
    ...
    if hero:  # 飞机击中判断
        ...
        enemy.isHitted(hero, plane_size[enemy.plane_type]["height"],
                       plane_size[enemy.plane_type]["height"])  # 是否击中enemy
...
def main():
    ...
    # 1. 创建窗口
    window_screen = pygame.display.set_mode((682, 800), 0, 32)
    # 2. 创建一个背景图片
    try:
        image_load()  # 数字图片等导入
    except Exception:
